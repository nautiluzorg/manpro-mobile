class CompletedTestingModel {
  final int count;
  final List<RecordTesting> results;

  CompletedTestingModel({
    required this.count,
    required this.results,
  });

  factory CompletedTestingModel.fromJson(Map<String, dynamic> json) {
    return CompletedTestingModel(
      count: json['count'] ?? 0,
...
class RecordTesting {
  ...
  final double cycleTimeMinutes; // baru
  ...
  factory RecordTesting.fromJson(Map<String, dynamic> json) {
    return RecordTesting(
      ...
      cycleTimeMinutes: (json['cycle_time_minutes'] ?? 0).toDouble(),
