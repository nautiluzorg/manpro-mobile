...
import 'package:flutter_provider_data/page/main_menu_simple.dart';
...
import 'package:flutter_provider_data/page/001-molding/report/recorddowntime.dart';
...
class _MenuSubState extends State<MenuSub> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<String, dynamic>> menus = [
      {
        "title": "MOLD COMPLETED",
        "icon": Icons.check_circle,
        "onTap": () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (_, __, ___) => RecordCompleted(
          ...
          Navigator.pushReplacement(
            ...
            PageRouteBuilder(
              pageBuilder: (_, __, ___) =>
                  RecordNg(title: "MOLDING NG", idProses: widget.idProses),
              transitionsBuilder: (_, animation, __, child) =>
                  FadeTransition(opacity: animation, child: child),
              transitionDuration: const Duration(milliseconds: 800),
            ),
      ...
      },
      {
        ...
        "onTap": () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (_, __, ___) => RecordDowntime(
                  title: "MOLDING DOWNTIME", idProses: widget.idProses),
              transitionsBuilder: (_, animation, __, child) =>
                  FadeTransition(opacity: animation, child: child),
              transitionDuration: const Duration(milliseconds: 800),
            ),
      ...
      },
      {
        ...
        "onTap": () {
          Navigator.pushReplacement(
            context,
            PageRouteBuilder(
              pageBuilder: (_, __, ___) => MainMenuSimple(title: "MAIN MENU"),
              transitionsBuilder: (_, animation, __, child) =>
                  FadeTransition(opacity: animation, child: child),
              transitionDuration: const Duration(milliseconds: 800),
            ),
