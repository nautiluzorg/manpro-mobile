...
// Tambah di bawah class JobNumberProvider, atau file terpisah

/*
  Future<void> scanJobNumber(BuildContext context, String idProses) async {

    try {
      isLoading = true;
      notifyListeners();

      // Reset dulu biar data lama tidak nyangkut
      _clearBeforeScan(context);
      ...
      if (scanned == null || scanned.isEmpty || scanned == "-1") return;

      // ============================
      // 2. Validate QR
      // ============================
      if (!_service.isValidQR(scanned)) {
        CustomSnackbar.show(context, "Invalid QR Code.", isSuccess: false);
        return;
      }
      ...
      // 3. Parse QR
      // ============================
      final parsed = _service.parseQR(scanned);
      ...
      jobDate = DateTime.now().toLocal().toString();
      jobProcess =
          "MOLDING"; // atau ambil dari parameter idProses jika mau fleksibel
      ...
      if (runStatus == "completed") {
        ...
        CustomSnackbar.show(context, "Jobnumber sudah finish.",
            isSuccess: false);
        notifyListeners();
        return;
      }
      ...
      if (runStatus == "pending") {
        _clearBeforeScan(context);
        notifyListeners();
        CustomSnackbar.show(context, "Jobnumber masih Pending.",
            isSuccess: false);
      ...
      final employeeData = statusData?["employee_finish"];
      if (employeeData != null) {
        _assignEmployee(employeeData); // update JobNumberProvider sendiri

        final employeeProvider =
            Provider.of<EmployeeProvider>(context, listen: false);
        final employee = EmployeeModel.fromJson(employeeData);

        employeeProvider.selectEmployee(employee);
      }
      ...
      final machineData = statusData?["machine"];
      if (machineData != null) {
        _assignMachine(machineData); // update JobNumberProvider sendiri
        final machineProvider =
            Provider.of<MachineProvider>(context, listen: false);
        machineProvider
            .setMachineDataFromMap(machineData); // update MachineProvider
      }
      ...
      // Update MaterialProvider secara global
      materialProvider.setMaterialFromJobNumber(
        mixLot: mix,
        goldGerman: goldGerman,
        goldUeda: goldUeda,
        goldMaterial: goldMaterial,
      ...
      // molds = await _service.getMoldsByDrawing(drawNumber);
      molds = await _service.getMoldsByDrawing(encodedDrawNumber);
      ...
      // CustomSnackbar.show(context, " QRCode Jobnumber berhasil discan!",isSuccess: true);
    } catch (e) {
      logPrint(
          "Error nya adalaaaaaaaaaaaaaaahhh: $e"); // Ini akan muncul di debug console

      CustomSnackbar.show(context, "Error nya : $e", isSuccess: false);
    } finally {
      isLoading = false;
      notifyListeners();
    }
